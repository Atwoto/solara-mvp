...
const statusFilters = ['All', 'Paid', 'Shipped', 'Delivered', 'Pending Payment', 'Processing', 'Cancelled'];
...
const AdminOrdersPage = () => {
    ...
    const filteredOrders = useMemo(() => {
        return orders
            .filter(order => {
                ...
                return order.status.replace(/_/g, ' ').toLowerCase() === activeStatus.toLowerCase();
            })
            .filter(order => {
    ...
    if (sessionStatus === 'loading' || isLoading) {
        return <div className="p-6"><PageLoader message="Loading orders..." /></div>;
    }
    if (sessionStatus !== 'authenticated' || session?.user?.email !== ADMIN_EMAIL) {
    ...
    return (
        <>
            ...
            <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200/80">
                        ...
                        {statusFilters.map(status => (
                            <button
                                ...
                                className={`relative px-4 py-2 text-sm font-semibold rounded-full whitespace-nowrap transition-colors duration-200 ${
                                    activeStatus === status ? 'text-white' : 'text-slate-600 hover:bg-slate-200/60'
                                }`}
                            >
                                {activeStatus === status && (
                                    <motion.div
                ...
                {/* Orders List */}
                <motion.div layout variants={containerVariants} initial="hidden" animate="visible" className="space-y-3">
                    <AnimatePresence>
                        {filteredOrders.length > 0 ? (
                            filteredOrders.map(order => (
                                <motion.div
                                    ...
                                    exit={{ opacity: 0, x: -50 }}
                                    key={order.id}
                                    className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center p-4 rounded-lg hover:bg-slate-50/70 transition-colors"
                                >
                                    <div className="md:col-span-4 font-mono text-xs text-slate-600">
                                        <span className="font-sans text-slate-400 text-xs">ID:</span> {order.id}
                                    </div>
                                    ...
                                    <div className="md:col-span-2 text-sm font-semibold text-slate-900 text-left md:text-right">
                                        Ksh {order.total_amount.toLocaleString()}
                                    </div>
                                    <div className="md:col-span-1 text-center">
                                        <span className={`px-2.5 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full border ${statusStyles[order.status.toLowerCase().replace(/ /g, '_')] || 'bg-gray-100 text-gray-800'}`}>
                                            {order.status.replace(/_/g, ' ')}
                                        </span>
                                    </div>
                                    <div className="md:col-span-1 text-right">
                                        <Link href={`/admin/orders/${order.id}`} className="p-2 rounded-lg text-slate-500 hover:bg-sky-100 hover:text-sky-600 transition-colors" title="View Order Details">
                            ...
                            <div className="text-center py-16 text-slate-500">
                                ...
                                <h3 className="mt-2 text-lg font-medium text-slate-800">No Orders Found</h3>
                                <p className="mt-1 text-sm">No orders match your current filter and search term.</p>
                            </div>
                        )}
                    </AnimatePresence>
                </motion.div>
