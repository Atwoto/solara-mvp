...
import { PencilSquareIcon, TrashIcon, PlusIcon, MagnifyingGlassIcon, CubeIcon } from '@heroicons/react/24/outline';
    ...
    return (
        <motion.div layout variants={itemVariants} exit="exit" className="bg-white rounded-xl shadow-sm border border-slate-200/80 flex flex-col">
            ...
            <div className="p-4 flex-grow flex flex-col">
                <p className="text-xs font-semibold text-solar-flare-end uppercase tracking-wider">{product.category}</p>
                ...
                <div className="mt-4 flex items-center justify-between pt-3 border-t border-slate-200/80">
                    ...
                    <div className="flex items-center gap-1">
                        ...
                        <button onClick={onDelete} className="p-2 rounded-lg text-slate-500 hover:bg-red-100 hover:text-red-600 transition-colors" title="Delete Product">
...
const AdminProductsPage = () => {
  ...
  return (
    <>
      ...
      >
        <Link href="/admin/products/new" className="bg-solar-flare-start hover:bg-solar-flare-end text-white font-semibold py-2 px-4 rounded-lg shadow-sm transition-colors duration-150 inline-flex items-center">
                      ...
                      {allFilters.map(cat => (
                          <button
                              ...
                              className={`relative px-4 py-2 text-sm font-semibold rounded-full whitespace-nowrap transition-colors duration-200 ${
                                  activeCategory === cat ? 'text-white' : 'text-slate-600 hover:bg-slate-200/60'
                              }`}
                          >
                              {activeCategory === cat && (
                                  <motion.div
                              ...
                              <span className="relative z-10">{cat.split('-').map(w=>w.charAt(0).toUpperCase()+w.slice(1)).join(' ')}</span>
                          </button>
                      ))}
                  </div>
              </div>
      ...
      {!isLoadingProducts && filteredProducts.length === 0 ? (
          <div className="text-center py-16 bg-white rounded-xl border-dashed border-2 border-slate-200">
              ...
              <p className="mt-1 text-sm text-slate-500">No products match your current search and filter combination.</p>
          </div>
      ) : (
         <motion.div
            layout
         ...
         >
            <AnimatePresence>
                {filteredProducts.map((product) => (
                    <ProductCard key={product.id} product={product} onDelete={() => handleDeleteProduct(product.id, product.name)} />
                ))}
            </AnimatePresence>
         </motion.div>
      )}
