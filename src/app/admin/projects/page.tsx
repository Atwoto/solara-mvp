    ...
    return (
        <motion.div layout variants={itemVariants} exit="exit" className="bg-white rounded-xl shadow-sm border border-slate-200/80 flex flex-col">
            ...
            <div className="p-4 flex-grow flex flex-col">
                <p className="text-xs font-semibold text-solar-flare-end">{project.category}</p>
                <h3 className="font-bold text-slate-800 mt-1 line-clamp-2" title={project.title}>{project.title}</h3>
                ...
                <div className="mt-4 flex items-center justify-between border-t pt-3">
                    ...
                    <div className="flex items-center gap-1">
                        <Link href={`/projects#${project.id}`} target="_blank" className="p-2 rounded-lg text-slate-500 hover:bg-sky-100 hover:text-sky-600 transition-colors" title="View Live Project">
                            <EyeIcon className="h-5 w-5"/>
                        </Link>
                        <Link href={`/admin/projects/edit/${project.id}`} className="p-2 rounded-lg text-slate-500 hover:bg-indigo-100 hover:text-indigo-600 transition-colors" title="Edit Project">
                            <PencilSquareIcon className="h-5 w-5"/>
                        </Link>
                        <button onClick={() => onDelete(project.id, project.title)} className="p-2 rounded-lg text-slate-500 hover:bg-red-100 hover:text-red-600 transition-colors" title="Delete Project">
...
export default function AdminProjectsPage() {
    ...
    if (sessionStatus === 'loading' || (isLoading && projects.length === 0)) {
        return <div className="p-6"><PageLoader message="Loading projects..." /></div>;
    }
     if (sessionStatus !== 'authenticated' || session?.user?.email !== ADMIN_EMAIL) {
    ...
    return (
        <div className="p-6 sm:p-8">
            ...
            >
                <Link href="/admin/projects/new" className="bg-solar-flare-start hover:bg-solar-flare-end text-white font-semibold py-2 px-4 rounded-lg shadow-sm transition-colors duration-150 inline-flex items-center">
            ...
            {!isLoading && filteredProjects.length === 0 ? (
                 <div className="text-center py-16 bg-white rounded-xl border-dashed border-2 border-slate-200">
                    ...
                    <p className="mt-1 text-sm text-slate-500">No projects match the filter "{activeFilter}".</p>
                </div>
            ) : (
                <motion.div
                    layout
