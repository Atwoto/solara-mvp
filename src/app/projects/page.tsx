      ...
      <main className="bg-gray-50 py-16 sm:py-20">
        <div className="container mx-auto px-4">
          <div className="flex justify-center items-center flex-wrap gap-2 sm:gap-4 mb-12">
            {filters.map(f => (
              <button key={f} onClick={() => setFilter(f)} className={`px-4 sm:px-6 py-2.5 text-sm font-semibold rounded-full transition-all duration-300 ${filter === f ? 'bg-deep-night text-white shadow-md' : 'bg-white text-gray-600 hover:bg-gray-200/50 hover:text-deep-night'}`}>{f}</button>
            ))}
          </div>
