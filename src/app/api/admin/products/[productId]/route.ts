...
export async function PUT(
  ...
  try {
    ...
    dataToUpdate.sold_count =
      soldCountStr && soldCountStr !== "" ? parseInt(soldCountStr) : null;
    dataToUpdate.rating =
      ratingStr && ratingStr !== "" ? parseFloat(ratingStr) : null;
    ...
    if (imageFiles.length > 0) {
      ...
      for (const result of uploadResults) {
        if (result.error)
          throw new Error(
            `Failed to upload new image(s): ${result.error.message}`
          );
      }
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json(
      { message: `Failed to archive product: ${error.message}` },
      { status: 500 }
    );
  }
}
