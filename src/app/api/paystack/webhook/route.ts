...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (event.event === "charge.success") {
      ...
      if (status !== "success") {
        console.log(
          `Charge status is '${status}', not 'success'. No action taken.`
        );
      ...
      if (findError || !order) {
        console.error(
          `Webhook Error: Order with reference ${reference} not found.`
        );
      ...
      if (
        ...
        console.log(
          `Order ${order.id} has already been processed. No action taken.`
        );
        return NextResponse.json({ received: true });
      }
      ...
      if (updateError) {
        console.error(
          `Webhook Error: Failed to update order ${order.id}.`,
          updateError
        );
        throw updateError;
      }
      ...
      console.log(
        `Received event '${event.event}', which is not 'charge.success'. No action taken.`
      );
    }
