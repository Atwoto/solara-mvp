...
import { supabase, supabaseAdmin } from '@/lib/supabaseClient'; // Use public or admin client as needed
import { Product as ProductTypeFromTypes, ServicePageData, BlogPost } from '@/types'; // Import all relevant types
...
// Helper function to strip HTML tags for cleaner context
...
export async function POST(req: Request) {
  ...
  // Use the admin client if available (for unpublished content if needed), otherwise fallback to public
  const dbClient = supabaseAdmin || supabase; 

  // --- START: Fetch ALL knowledge base content ---
  ...
  try {
    // Using Promise.all to fetch all knowledge concurrently for better performance
    ...
    if (productsRes.data && productsRes.data.length > 0) {
      productKnowledge = productsRes.data.map(p => 
    ...
    if (servicesRes.data && servicesRes.data.length > 0) {
      serviceKnowledge = servicesRes.data.map(s => 
    ...
    if (articlesRes.data && articlesRes.data.length > 0) {
      articleKnowledge = articlesRes.data.map(a => 
    ...
    // The chatbot will proceed with default "not available" messages for knowledge
  }
  // --- End of knowledge base fetching ---

  
  // --- Format Cart and Wishlist knowledge (your existing logic is fine) ---
  ...
  if (wishlist && wishlist.length > 0) {
    try {
        const { data: wishlistProducts, error: wishlistError } = await dbClient // Use dbClient for consistency
            .from('products')
            .select('id, name')
            .in('id', wishlist);
  ...
  // --- Updated System Prompt with all knowledge sections ---
  const systemPrompt = `
