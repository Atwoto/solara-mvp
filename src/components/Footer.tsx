...
// --- STEP 1: Import all the necessary icons ---
...
const Footer = () => {
  ...
  const handleSubscribe = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setSubmitMessage(result.message || "Subscription failed.");
  ...
  // --- STEP 2: Update the socialLinks array with your information ---
  const socialLinks = [
    ...
    // You can add your X/Twitter and Instagram links here when you have them
    ...
    <footer className="relative bg-deep-night text-gray-300">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-20">
          ...
          <div className="lg:col-span-5 space-y-6">
            ...
            <div className="pt-2">
              ...
              <form onSubmit={handleSubscribe} className="relative flex items-center">
                  <input type="email" placeholder="Your email address" aria-label="Your email address" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full pl-4 pr-12 py-3 text-sm text-gray-900 bg-gray-200 rounded-lg focus:ring-2 focus:ring-solar-flare-end focus:border-solar-flare-end outline-none border border-transparent" />
                  <button type="submit" aria-label="Subscribe" disabled={isSubmitting} className="absolute inset-y-1 right-1 flex items-center justify-center aspect-square w-10 text-white bg-solar-flare-end rounded-md hover:bg-solar-flare-start disabled:bg-gray-400 transition-colors">
                      {isSubmitting ? (<svg className="animate-spin h-5 w-5" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path></svg>) : (<PaperAirplaneIcon className="h-5 w-5 -rotate-45" />)}
                  </button>
              </form>
              {submitMessage && (<p className={`mt-2 text-xs font-medium ${submitMessageType === 'success' ? 'text-green-400' : 'text-red-400'}`}>{submitMessage}</p>)}
            </div>
          </div>
          ...
          <div className="lg:col-span-6 grid grid-cols-1 md:grid-cols-2 gap-10">
            <div>
              ...
              <ul className={`space-y-3 overflow-hidden transition-all duration-300 ease-in-out md:max-h-none mt-5 ${openAccordion === 'quickLinks' ? 'max-h-60' : 'max-h-0'}`}>
                {quickLinks.map(link => ( <li key={link.href}><Link href={link.href} className="hover:text-solar-flare-start transition-colors">{link.label}</Link></li> ))}
              </ul>
            </div>
              ...
              <div className={`space-y-4 not-italic overflow-hidden transition-all duration-300 ease-in-out md:max-h-none mt-5 ${openAccordion === 'contact' ? 'max-h-48' : 'max-h-0'}`}>
                {contactInfo.map(item => (
                  <div key={item.label} className="flex items-start space-x-3">
                    ...
                    {item.type === 'link' ? (<Link href={item.href!} className="hover:text-solar-flare-start transition-colors">{item.label}</Link>) : (<span>{item.label}</span>)}
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="mt-16 border-t border-gray-800 pt-8 flex flex-col-reverse md:flex-row items-center justify-between gap-6">
          <p className="text-xs text-gray-500">© {currentYear} Bills On Solar EA Limited. All Rights Reserved.</p>
