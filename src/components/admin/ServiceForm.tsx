...
const statusOptions: ServicePageData["status"][] = [
  "draft",
  "published",
  "archived",
];
...
type ServiceFormState = Omit<
  ServicePageData,
  "id" | "created_at" | "updated_at" | "image_urls" | "features"
> & {
  featuresJson: string;
  imageFiles: File[];
  ...
  return categories.map((category) => {
    ...
    return (
      <Fragment key={category.id}>
        ...
        {children.length > 0 &&
          renderCategoryOptions(children, allCategories, level + 1)}
      </Fragment>
    );
  });
};
  ...
  const getInitialFormState = useCallback(
    (): ServiceFormState => ({
      ...
      content_html:
        ...
        "<p>Start writing your service details here!</p>",
  ...
  const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      if (validFiles.length > 0) {
        ...
        if (
          error &&
          (error.includes("not a valid image type") ||
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="p-6 bg-white rounded-xl shadow-sm border border-slate-200/80">
            ...
            <select
              ...
              required
              className="mt-1 block w-full px-3 py-2.5 border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-solar-flare-start focus:border-solar-flare-start sm:text-sm"
            >
              <option value="" disabled>
                -- Select a service to pre-fill --
              </option>
            ...
            <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        {imagePreviews.map((preview, index) => (
                          ...
                          >
                            ...
                            <div
                              className="absolute bottom-1 right-1 bg-black/75 text-white text-xs px-1 py-0.5 rounded max-w-[80px] truncate"
              ...
              {(formData.currentImageUrls.length > 0 ||
                ...
                <p className="text-sm text-slate-600 bg-slate-50 p-2 rounded">
                  📷 {formData.currentImageUrls.length} existing,{" "}
                  {imagePreviews.length} new images
                </p>
              )}
            </div>
          </SettingsCard>
