...
import { XCircleIcon } from '@heroicons/react/24/solid'; // We need this for the remove buttons
...
// --- STATE UPGRADE: To handle multiple images, same as ProductForm ---
type ServiceFormState = Omit<ServicePageData, 'id' | 'created_at' | 'updated_at' | 'image_urls' | 'features'> & {
    featuresJson: string;
    imageFiles: File[];       // Changed from single to array
    currentImageUrls: string[]; // Changed from single to array
};
...
export default function ServiceForm({ initialData, onSubmitSuccess }: ServiceFormProps) {
  ...
  // --- Image Handlers (copied from ProductForm logic) ---
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 bg-white p-6 md:p-8 rounded-lg shadow-md">
      ...
      <div>
        <label htmlFor="content_html" className="block text-sm font-medium text-gray-700 mb-1">Service Content <span className="text-red-500">*</span></label>
        <TipTapEditor content={formData.content_html} onChange={handleContentChange} />
      </div>
      
      {/* --- REPLACED WITH PRODUCT FORM STYLE IMAGE UPLOAD UI --- */}
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">Service Images</label>
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-4 p-4 border rounded-lg bg-gray-50 min-h-[120px]">
          {formData.currentImageUrls.map((url) => (
            <div key={url} className="relative group aspect-square">
              <Image src={url} alt="Existing image" fill className="object-cover rounded-md border" sizes="15vw"/>
              <button type="button" onClick={() => removeExistingImage(url)} className="absolute -top-2 -right-2 bg-white rounded-full transition-transform hover:scale-110"><XCircleIcon className="h-6 w-6 text-red-500 hover:text-red-700" /></button>
            </div>
          ))}
          {imagePreviews.map((previewUrl, index) => (
            <div key={index} className="relative group aspect-square">
              <Image src={previewUrl} alt="New preview" fill className="object-cover rounded-md border" sizes="15vw"/>
              <button type="button" onClick={() => removeNewImage(index)} className="absolute -top-2 -right-2 bg-white rounded-full transition-transform hover:scale-110"><XCircleIcon className="h-6 w-6 text-red-500 hover:text-red-700" /></button>
            </div>
          ))}
        </div>
        <div className="mt-4">
          <label htmlFor="imageFiles" className="w-full sm:w-auto cursor-pointer inline-flex justify-center items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-solar-flare-start hover:bg-solar-flare-end focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-solar-flare-start">
            Choose Files
          </label>
          <input 
            type="file" 
          ...
          />
          <span className="ml-3 text-sm text-gray-500">{formData.imageFiles.length > 0 ? `${formData.imageFiles.length} file(s) selected` : "No new files chosen"}</span>
        </div>
        <p className="text-xs text-gray-500 mt-2">You can add multiple images. Max 2MB per file.</p>
      </div>
