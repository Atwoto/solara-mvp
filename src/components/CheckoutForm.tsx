...
export default function CheckoutForm() {
  ...
  useEffect(() => {
    if (status === "unauthenticated" && !guestSession) {
      // Show guest checkout modal only if no guest session exists
      setShowGuestModal(true);
    } else if (session?.user?.name) {
      setFormData((prev) => ({ ...prev, fullName: session.user.name! }));
    } else if (guestSession) {
      // Pre-fill guest email if guest session exists
      setFormData((prev) => ({ ...prev, guestEmail: guestSession.email }));
    }
  }, [status, router, session, guestSession]);
  ...
  const handlePayment = async (e: FormEvent) => {
    ...
    try {
      // Determine user email - either from session, guest session, or form
      const userEmail =
        session?.user?.email || guestSession?.email || formData.guestEmail;
      ...
      const handler = window.PaystackPop.setup({
        ...
        callback: (response: any) => {
          clearCart();
          router.push(
            `/order-confirmation?orderId=${checkoutData.orderId}&ref=${response.reference}`
          );
        },
      });
      handler.openIframe();
  ...
  if (status === "loading" || isCartLoading) {
    return (
      <div className="flex h-screen items-center justify-center">
        <p className="text-lg text-gray-600">Loading your secure checkout...</p>
      </div>
    );
  }
  ...
  if (cartItems.length === 0 && !isProcessingPayment) {
    return (
      <div className="text-center py-16 bg-white rounded-xl shadow-lg container mx-auto mt-12">
        ...
        <Link
          href="/products"
          className="inline-block px-8 py-3 text-base font-semibold text-deep-night bg-gradient-to-r from-solar-flare-start to-solar-flare-end rounded-lg shadow-md hover:shadow-lg transition-transform hover:scale-105"
        >
          Browse Products
        </Link>
      </div>
      ...
      <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12">
            ...
            >
              ...
              <div className="space-y-6">
                ...
                <FormInput
                  ...
                  name="email"
                  type="email"
                  value={session?.user?.email || formData.guestEmail || ""}
                ...
                <div>
                  ...
                  <div className="relative">
                    <div className="pointer-events-none absolute inset-y-0 left-0 top-0 flex items-center pl-3.5 pt-3.5">
                    ...
                    <textarea
                      ...
                      onChange={handleInputChange}
                      rows={3}
                      className="block w-full rounded-lg border-gray-300 shadow-sm p-3 pl-10 focus:border-solar-flare-start focus:ring-solar-flare-start sm:text-sm transition-colors"
                      required
            ...
            >
              <div className="rounded-2xl border bg-white p-6 shadow-lg sticky top-28">
                <h2 className="text-xl sm:text-2xl font-semibold text-graphite mb-6 pb-4 border-b">
                  Your Order
                </h2>
                  ...
                  {cartItems.map((item) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <span className="font-medium text-graphite block line-clamp-1">
                            {item.name}
                          </span>
                ...
                <div className="p-3 bg-blue-50 text-blue-800 rounded-lg text-xs flex items-start gap-2.5 mb-5 border border-blue-200">
                  ...
                  {/* --- THIS IS THE FIX --- */}
                  <span>
                    All shipping and delivery costs will be communicated after
                ...
                >
                  {isProcessingPayment ? (
                    ...
                    <>
                      <LockClosedIcon className="h-5 w-5 mr-2" /> Proceed to
                      Payment
                    </>
                  )}
                </button>
        ...
        <GuestCheckoutModal
          ...
          onGuestContinue={(email) => {
            ...
            setGuestSession({ email, isGuest: true });
