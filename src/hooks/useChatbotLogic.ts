...
const createWelcomeMessage = (): Message => ({
  ...
  role: "assistant",
  content:
    "Hello! I'm the Bills On Solar Assistant. I can help you find solar products, answer questions about solar energy, and even manage your cart or wishlist. How can I assist you today?",
});
...
export const useChatbotLogic = () => {
  ...
  const { cartItems, addToCart, removeFromCart, clearCart, openCart } =
    useCart();
  const { wishlistIds, addToWishlist, removeFromWishlist, clearWishlist } =
    useWishlist();
  ...
  const handleActionClick = useCallback(
    async (actionType: string, productIdOrMarker: string) => {
      const normalizedActionType = actionType.replace(/_([a-z])/g, (g) =>
        g[1].toUpperCase()
      );
      const productSpecificActions = [
        "addToCart",
        "addToWishlist",
        "removeFromCart",
        "removeFromWishlist",
      ...
      if (productSpecificActions.includes(normalizedActionType)) {
        if (
          !productIdOrMarker ||
          productIdOrMarker.toLowerCase() === "undefined" ||
          productIdOrMarker === "NONE"
        ) {
      ...
      try {
        ...
        console.error(
          `Error performing chatbot action '${actionType}':`,
          err.message
        );
  ...
  const {
    ...
    onError: (err) => {
      ...
      showNotificationMessage(
        "Apologies, I couldn't send that. Please try again.",
        "error"
      );
    },
  });
